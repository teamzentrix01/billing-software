  ...
  try {
    ...
    for (const item of normalizedItems) {
      ...
      const discountAmount =
        allowDiscounts && item.dbProduct?.allow_discount_on_pos
    ...
    const allowOrderDiscount =
      ...
      normalizedItems.every((item) => item.dbProduct?.allow_discount_on_pos);
    const totalDiscount =
      (allowOrderDiscount ? toNumber(orderDiscount) : 0) +
    ...
    if (Math.abs(paidAmount - grandTotal) > 0.01) {
      await client.query("ROLLBACK");
      return errorResponse(
        `Payment total must match bill total. Paid ${paidAmount}, bill ${grandTotal}`,
        400,
      );
    }
  ...
  try {
    ...
    let productsSql = `
      SELECT
        ...
        p.barcode,
        p.unit,
        COALESCE(batch_variant.variant_mrp, p.mrp, 0) AS mrp,
        COALESCE(batch_variant.variant_selling_price, NULLIF(ps.selling_price, 0), p.selling_price, 0) AS selling_price,
